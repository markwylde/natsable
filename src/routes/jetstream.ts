...
export function createJetstreamRouter(natsUrl: string, certsDir: string) {
  ...
  router.get('/streams', async (req, res) => {
    try {
      ...
      for await (const stream of jsm.streams.list()) {
        ...
        if (
          stream.config.name.startsWith('KV_') ||
          stream.config.name.startsWith('OBJ_')
        ) {
          continue;
        }
      ...
      res
        .status(500)
        .json({ error: 'Failed to list streams', message: error.message });
    }
  });
    ...
    try {
      ...
      if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
        return res.status(400).json({
          error:
            'Stream name can only contain alphanumeric characters, underscores, and dashes',
        });
      }
      ...
      if (maxMsgs) config.max_msgs = Number.parseInt(maxMsgs);
      if (maxBytes) config.max_bytes = Number.parseInt(maxBytes);
      if (maxAge) config.max_age = Number.parseInt(maxAge); // nanoseconds
      ...
      res
        .status(500)
        .json({ error: 'Failed to create stream', message: error.message });
    }
  });
  ...
  router.delete('/streams/:name', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to delete stream', message: error.message });
    }
  });
  ...
  router.get('/streams/:name', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to get stream info', message: error.message });
    }
  });
  ...
  router.get('/streams/:name/consumers', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to list consumers', message: error.message });
    }
  });
  ...
  router.post('/streams/:name/consumers', async (req, res) => {
    try {
      ...
      if (maxDeliver) config.max_deliver = Number.parseInt(maxDeliver);
      if (ackWait) config.ack_wait = Number.parseInt(ackWait);
      ...
      res
        .status(500)
        .json({ error: 'Failed to create consumer', message: error.message });
    }
  });
  ...
  router.delete('/streams/:name/consumers/:consumer', async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `Consumer "${consumer}" deleted successfully`,
      });
    } catch (error: any) {
      console.error('Error deleting consumer:', error);
      res
        .status(500)
        .json({ error: 'Failed to delete consumer', message: error.message });
    }
  });
  ...
  router.get('/streams/:name/consumers/:consumer', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to get consumer info', message: error.message });
    }
  });
  ...
  router.get('/streams/:name/messages', async (req, res) => {
    try {
      ...
      const effectiveLimit = Math.min(Number.parseInt(limit as string), 100);
      const start = startSeq
        ? Number.parseInt(startSeq as string)
        : Math.max(firstSeq, lastSeq - effectiveLimit + 1);

      // Fetch messages
      for (
        let seq = start;
        seq <= lastSeq && messages.length < effectiveLimit;
        seq++
      ) {
        try {
  ...
  router.get('/streams/:name/messages/:seq', async (req, res) => {
    try {
      ...
      const stream = await js.streams.get(name);
      const msg = await stream.getMessage({ seq: Number.parseInt(seq) });
      ...
      res
        .status(500)
        .json({ error: 'Failed to get message', message: error.message });
    }
  });
  ...
  router.post('/streams/:name/publish', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to publish message', message: error.message });
    }
  });
  ...
  router.post('/streams/:name/purge', async (req, res) => {
    try {
      ...
      if (seq) opts.seq = Number.parseInt(seq);
      if (keep) opts.keep = Number.parseInt(keep);
      ...
      res
        .status(500)
        .json({ error: 'Failed to purge stream', message: error.message });
    }
  });
  ...
  router.delete('/streams/:name/messages/:seq', async (req, res) => {
    try {
      ...
      await jsm.streams.deleteMessage(
        name,
        Number.parseInt(seq),
        noErase !== 'true',
      );
